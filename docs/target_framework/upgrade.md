...
### 4a. `/dashboard` now leads into the panel

**What changed.** A signed-in user who lands on the starter kit's `/dashboard`
is redirected to the first panel they can enter. It is the counterpart to the
guest redirect above: that one sends a guest out to the right login, this one
sends a signed-in user into the panel they would otherwise have had to find by
typing its URL.

**What breaks.** Your `/dashboard` route, its name, and
`resources/js/pages/Dashboard.vue` are all untouched — the request is simply
answered earlier by a fourth `web` middleware. Nothing that links to the route
name changes. What breaks is an application whose `/dashboard` is a real screen
somebody uses, because it is now unreachable while signed in.

**If yours is a real screen**, turn it off. This applies to an existing install
too: the key is merged from the package, so publishing your config before this
release does not opt you out.
...
Or hand over a different path instead — the values are `Request::is()`
patterns, and a path a panel is itself mounted on is ignored:
...
### 4b. Schema mistakes are refused instead of ignored

**What changed.** Nine things a table, form, action or widget definition could
say that cannot be true now raise `PandaPanel\Exceptions\PanelSchemaException`:
duplicate column names, duplicate field names, duplicate filter names, duplicate
action names within one set, an action with nothing to do, `defaultSort()` on a
column the table does not have, an empty name on a column, field or action, a
widget column span that is neither a number nor `'full'`, and a widget column
span at a breakpoint the grid does not have.

**What breaks.** Nothing that was working. Every one of these produced wrong
behaviour rather than no behaviour — a duplicate column name meant two columns
sharing one cell value and one sort; a duplicate field name meant one of the
two fields was submitted and discarded; a duplicate action name meant the
second button always ran the first action.

**What it costs.** An application carrying one of these gets an exception where
it previously had a quiet bug. The check runs when the schema is built, which
is at boot or on the first render of the page — so `php artisan test` finds it,
not a user. The message names the schema, the name, and the fix.

If a refusal is wrong for your case, the fix is in the schema rather than in
config: rename one of the two, use `dehydrateTo()` for two inputs that really do
write one column, or delete the action that does nothing.
