...
Two starter kit addresses stop being screens of their own, and both stay
addresses:
...
| `/settings/*` | Nothing in the package. The example application redirects them into the panel's own settings pages — see `SettingsRedirectController`. | Do not copy that controller |

Nothing else the application owns is read, edited, or overwritten. Everything
this package publishes lands under `resources/js/panel`, `resources/js/pages`,
and the paths in `PublishedAssets::map()`.
