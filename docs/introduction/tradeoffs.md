...
The published components import **eighteen modules they do not ship**, and both kinds are the
application's on purpose: `@/routes/*` and `@/actions/*` are generated by Wayfinder from your own
route table, and the rest — `@/components/UserMenuContent.vue`, `@/composables/useTwoFactorAuth`,
six more — are where a project keeps its own account UI.

In practice: **a Laravel Vue starter kit application works out of the box; anything else needs those
eighteen files written first.** `panel:install` checks for all of them and names the ones missing.
